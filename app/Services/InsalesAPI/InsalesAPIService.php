...
class InsalesAPIService extends BaseModelService
{
    ...
    {
        $obj1 = DB::table('warehouse_products_amounts')->where('warehouse_id', 1)->where('product_id', $productId)->orderBy('updated_at', 'DESC')->first();
        $obj2 = DB::table('warehouse_products_amounts')->where('warehouse_id', 2)->where('product_id', $productId)->orderBy('updated_at', 'DESC')->first();
